...
///全  局  内  容
vector<Text> text; //定义一个Text类的容器，存储文本内容
Text one_Content;
    ...
    if (in) //文件成功打开
    {
        while (getline(in, line))
        {
            ...
            else
            {
                ...
                text.push_back(one_Content); //存入容器
            }
        }
    }
    else
        cout << "该文件不存在:" << book << endl;
}
...
void search(string info, vector<Text> &s) //查找信息
{
...
void show_Info(vector<Text> s, string info) //显示查询结果
{
...
int main()
{
    ...
    vector<Text> search_Info; //存储搜索到的信息
    cout << "请输入要查询的 人名/地名: " << endl;
    getline(cin, input_Info); //获取输入
    search(input_Info, search_Info);
